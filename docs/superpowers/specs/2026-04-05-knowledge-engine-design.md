...
slug 生成规则：项目根路径规范化（去掉前导 `/Users/`，路径分隔符替换为 `-`）。示例：`/Users/zhushanwen/GitApp/my-project` -> `gitApp-my-project`。
...
- 过滤：排除 lock 文件、node_modules、.env、knowledge 目录自身
- 变更量极小（纯空白变更）也跳过
...
4. 对每个 commit：
   - 取 diff（`git diff <commit>~1 <commit>`，初始 commit 用 `git show`）
   - 读取 changelog.log 中该时间范围内的条目
...
```typescript
const command = process.argv[2] // record | summarize | consolidate | inject-index
```
...
```json
{
  ...
  "Stop": [{
    "hooks": [
      {
        "type": "command",
        "command": "bun /path/to/knowledge-engine/src/cli.ts summarize",
        "async": true,
        "timeout": 60
      },
      {
        "type": "command",
        "command": "bun /path/to/knowledge-engine/src/cli.ts consolidate",
        "async": true,
        "timeout": 120
      }
    ]
...
清理过旧的 changelog 已处理条目。实现上可以在 consolidate 后截断 changelog.log 中已被总结的部分。
...
| 多会话并发 | changelog 追加天然安全，state.json 原子 rename |
...
| `Stop` hook | summarize -> consolidate | 模型停止时，内部有守卫条件 |
| crontab | summarize + consolidate + cleanup | 定时全量维护 |
