...
源目录:
  - backend/app/infra/completeness/providers/collection/
  - backend/app/infra/completeness/providers/calculation/
...
使用方法:
    ...
    python .refactor/20260316-110000-move-providers-to-supporting.py --dry-run
    python .refactor/20260316-110000-move-providers-to-supporting.py
"""
...
def main():
    ...
    # 定义文件移动映射: 源文件路径 -> 目标目录路径
    # 注意：MoveModule 需要目标目录（必须是包），而不是完整文件路径
    moves = {
        # Collection providers -> backend/app/domain/supporting/collection/
        "backend/app/infra/completeness/providers/collection/__init__.py":
            "backend/app/domain/supporting/collection",
        ...
        "backend/app/infra/completeness/providers/calculation/__init__.py":
            "backend/app/domain/supporting/calculation",
    ...
    print(f"{'[预览模式] ' if dry_run else ''}开始移动 {len(moves)} 个文件...")
    print("-" * 60)
