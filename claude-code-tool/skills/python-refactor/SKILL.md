...
rope 是重量级工具，默认配置下可能非常慢。以下 3 个优化建议可以显著提升性能：
...
**问题**：每次修改都会触发缓存失效，批量操作时影响显著

```python
# ✅ 批量操作时暂停 observer
try:
    project.pycore.observer.pause()  # 暂停资源观察

    # 执行多个重构操作
    changes1 = renamer1.get_changes("new_name")
    project.do(changes1)

    changes2 = mover.get_changes(dest_folder)
    project.do(changes2)

finally:
    project.pycore.observer.resume()  # 恢复观察
    project.close()
```
...
try:
    ...
    project.pycore.observer.resume()
    project.close()
```
